...
class HDFreaksWeather(Converter, object):
...
class WeatherData:
	def __init__(self):
		...
		if config.plugins.HDFreaks.refreshInterval.value > 0:
			self.timer = eTimer()
	...
	def GetWeather(self):
		timeout = config.plugins.HDFreaks.refreshInterval.value * 1000 * 60
		if timeout > 0:
			self.timer.start(timeout, True)
			print "HDFreaks lookup for ID " + str(config.plugins.HDFreaks.weather_city.value)
			url = "http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%3D%22"+str(config.plugins.HDFreaks.weather_city.value)+"%22&format=xml"
